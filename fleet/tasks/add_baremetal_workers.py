...
CLI: fleet-add-baremetal-workers --cluster-name NAME
...
def create_baremetal_machinepool(
    ...
    # Apply the MachinePool via stdin (JSON format)
    try:
        machinepool_json = json.dumps(machinepool_spec)
    except (TypeError, ValueError) as e:
        error(f"Failed to serialize MachinePool JSON: {e}")
        return False

    apply_result = run_with_retry(
        ["oc", "apply", "-f", "-"],
        input=machinepool_json,
        capture_output=True,
        text=True,
    )
