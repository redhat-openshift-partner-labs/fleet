"""Add GPU workers to existing cluster via MachinePool creation on hub.

CLI: fleet-add-gpu-workers --cluster-name NAME --pipeline-image IMAGE
Creates additional GPU worker MachinePool for AI workloads.
Exits 1 on failure.
"""
...
def create_gpu_machinepool(
    cluster_name: str, region_info: Dict[str, Any]
) -> bool:
    """Create a new MachinePool for GPU workers."""
    mp_name = f"{cluster_name}-gpu"
    info(f"Creating GPU MachinePool: {mp_name}")
    ...
    machinepool_spec = {
        ...
        "metadata": {
            ...
            "labels": {"cluster": cluster_name, "tier": "ai"},
        },
        "spec": {
            ...
            "platform": {
                "aws": {
                    "rootVolume": {
                        "iops": 4000,
                        "size": 256,  # Larger disk for AI workloads
                        "type": "gp3",
                    },
                    "type": "g5.4xlarge",  # GPU instance for AI workloads
                    "zones": region_info["zones"],
                }
            },
            "replicas": 1,  # Start with one GPU worker
            "labels": {
                "node-role.kubernetes.io/worker": "",
                "node-role.kubernetes.io/gpu": "",
                "fleet.openshift.com/tier": "ai",
            },
            "taints": [
                {
                    "effect": "NoSchedule",
...
def wait_for_machinepool_ready(cluster_name: str, timeout: int = 2400) -> bool:
    """Wait for the GPU MachinePool to provision nodes."""
    mp_name = f"{cluster_name}-gpu"
...
def main() -> None:
    ...
    configure("add-gpu-workers")

    info("=== Adding GPU workers to cluster ===")
    info(f"Parameters:")
    ...
    # Create GPU MachinePool
    if not create_gpu_machinepool(cluster, region_info):
        sys.exit(1)
    ...
    info("GPU workers successfully added to cluster")